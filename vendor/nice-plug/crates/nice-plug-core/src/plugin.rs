...
pub trait Plugin: Default + Send + 'static {
    ...
    /// Called when the shared library is about to be unloaded (`ExitDll` /
    /// `DLL_PROCESS_DETACH` on Windows, `ModuleExit` / `bundleExit` elsewhere).
    ///
    /// If the plugin spawned background threads that live inside the library,
    /// they MUST be stopped here: once the host unloads the library its code is
    /// unmapped, and any thread still executing code from it (or any window
    /// whose `WndProc` lives in it) will crash the host with an access
    /// violation. Note that this runs while the loader lock is held, so the
    /// implementation must not load libraries or block on anything that may.
    ///
